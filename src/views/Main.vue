...
async function getData(logId?: string) {
    try {
        ...
        const res: any = await axios.get(
            ...
            `https://admin.lightning.ai.kr/api/mission/test/info`,
        )
    ...
    <section class="section">
        ...
        " @click="nextVideo">
            <video preload="auto" playsinline webkit-playsinline @ended="handleVideoEnd(i)"
                v-for="(url, i) in missionStore.data.screenType === '1' ? videoUrls : videoUrls2" :key="url"
                v-show="i === videoTrigger" muted autoplay ref="videoRefs"
